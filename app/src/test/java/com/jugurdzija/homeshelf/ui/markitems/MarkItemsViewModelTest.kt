...
class MarkItemsViewModelTest {
    ...
    private lateinit var manageStorageUseCase: ManageStorageUseCase
    ...
    fun setUp() {
        ...
        coEvery { getStorageUseCase.getStorage(any()) } returns
        ...
        coEvery { manageStorageUseCase.saveMarkedItems(any(), any()) } just Runs
    }
    ...
    private fun createViewModel(): MarkItemsViewModel {
        ...
        val viewModel = MarkItemsViewModel(
            savedStateHandle, getStorageUseCase, getStorageReferenceUseCase, manageStorageUseCase, itemDetector, mockk<AudioRecorder>(), mockk<SpeechToTextEngine>()
        )
    ...
    fun `confirmSelection with a name keeps the item, clears selection and persists`() = runTest(testDispatcher) {
        ...
        assertNull(viewModel.selectedId)
        coVerify { manageStorageUseCase.saveMarkedItems(STORAGE_ID, viewModel.markedItems.toList()) }
    }
    ...
    fun `deleteItem removes the item, clears selection if selected, and persists`() = runTest(testDispatcher) {
        ...
        assertTrue(viewModel.markedItems.isEmpty())
        assertNull(viewModel.selectedId)
        coVerify { manageStorageUseCase.saveMarkedItems(STORAGE_ID, emptyList()) }
    }
