...
object Routes {
    ...
    const val SCAN = "$SCAN_BASE?$ARG_STORAGE_ID={$ARG_STORAGE_ID}"
    ...
    fun scan(storageId: String? = null) =
        if (storageId != null) SCAN.replace("{$ARG_STORAGE_ID}", storageId) else SCAN_BASE
